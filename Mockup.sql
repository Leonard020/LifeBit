...
('벤치프레스', 'strength', 'chest', '가슴 운동의 대표적인 운동', 'high'),
('스쿼트', 'strength', 'legs', '하체 운동의 기본', 'high'),
('데드리프트', 'strength', 'back', '등과 하체를 동시에 운동', 'high'),
('플랭크', 'strength', 'abs', '코어 강화 운동', 'medium'),
('풀업', 'strength', 'back', '등 근육 강화 운동', 'high'),
('조깅', 'aerobic', 'cardio', '기초 체력 향상을 위한 유산소 운동', 'medium');
...
((SELECT user_id FROM users WHERE email = 'admin@lifebit.com'), 
 ...
 45, 300, 80, 12, 3, CURRENT_DATE, 'morning', 'TYPING', NULL, 'VALIDATED'),
((SELECT user_id FROM users WHERE email = 'user1@example.com'), 
 (SELECT exercise_catalog_id FROM exercise_catalog WHERE name = '스쿼트'), 
 30, 250, 100, 15, 4, CURRENT_DATE, 'afternoon', 'VOICE', 0.85, 'VALIDATED'),
((SELECT user_id FROM users WHERE email = 'user2@example.com'), 
 (SELECT exercise_catalog_id FROM exercise_catalog WHERE name = '데드리프트'), 
 40, 280, 120, 10, 3, CURRENT_DATE, 'dawn', 'VOICE', 0.92, 'VALIDATED'),
((SELECT user_id FROM users WHERE email = 'user3@example.com'), 
 (SELECT exercise_catalog_id FROM exercise_catalog WHERE name = '플랭크'), 
 20, 150, 0, 1, 3, CURRENT_DATE, 'night', 'TYPING', NULL, 'VALIDATED'),
((SELECT user_id FROM users WHERE email = 'user4@example.com'), 
 (SELECT exercise_catalog_id FROM exercise_catalog WHERE name = '풀업'), 
 35, 270, 0, 8, 4, CURRENT_DATE, 'morning', 'VOICE', 0.78, 'PENDING');
